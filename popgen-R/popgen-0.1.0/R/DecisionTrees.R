...
#' @export
dtPrediction <- function(train.data.set, test.data.set, labels = c("Region"), categorical.var = c("SampleId"), cp = 0.001, split.cr = "gini") {
  theObject <- dtObj()
  ...
  model = rpart(Region ~ ., data = data, parms = list(split = split.cr), control = rpart.control(cp = cp, xval = 10, maxdepth = 10))
