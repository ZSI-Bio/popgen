...
svmEstimation <- function(train.data.set,
                                ...
                                which.kernel = "RBF",  costs = c(10, 100, 1000, 10000), sigmas = 10^(-2:1)){
...
#' SVM prediction
#' @export
svmPrediction <- function(train.data.set, test.data.set, labels = c("Region"), categorical.var = c("SampleId"),  which.kernel = "RBF", cost = 100, sigma = 1){
            theObject <- svmObj()
            theObject@cost <- cost
