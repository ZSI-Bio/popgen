...
#' @export
plotTuning <- function(cross.tbl, xlabel = "Threshold", ylabel = "Error", isNumeric = F){
  ...
  if (isNumeric) plotdf[, "value"] <- as.numeric(as.character(plotdf[, "value"]))

  ggplot(plotdf, aes(Threshold, value, colour = series, group = series)) +
    geom_line(size = 0.5) +
    geom_point() +
